...
export const validateCreateUser = [
  body('credential_id').notEmpty().withMessage('Credential ID tidak boleh kosong.'),
  ...
  body('date_of_birth').isISO8601().toDate().withMessage('Format tanggal lahir tidak valid (YYYY-MM-DD).'),
  body('address').notEmpty().withMessage('Alamat tidak boleh kosong.'),
  body('contribution').isIn(CONTRIBUTIONS).withMessage('Tipe kontribusi tidak valid.'),
...
export const validateUpdateUser = [
  ...
  body('credential_id').optional().notEmpty().withMessage('Credential ID tidak boleh kosong.'),
  body('nickname').optional().notEmpty().withMessage('Nama panggilan tidak boleh kosong.'),
  body('phone_number').optional().isMobilePhone('id-ID').withMessage('Format nomor telepon tidak valid.'),
  ...
  body('date_of_birth').optional().isISO8601().toDate().withMessage('Format tanggal lahir tidak valid.'),
  body('address').optional().notEmpty().withMessage('Alamat tidak boleh kosong.'),
  body('contribution').optional().isIn(CONTRIBUTIONS).withMessage('Tipe kontribusi tidak valid.'),
  body('village').optional().notEmpty().withMessage('Desa/Kelurahan tidak boleh kosong.'),
  body('subdistrict').optional().notEmpty().withMessage('Kecamatan tidak boleh kosong.'),
  body('post_code').optional().notEmpty().withMessage('Kode pos tidak boleh kosong.'),
  body('no_nik').optional().isLength({ min: 16, max: 16 }).withMessage('NIK harus 16 digit.'),
  body('photo_ktp').optional().isURL().withMessage('URL foto KTP tidak valid.'),
];